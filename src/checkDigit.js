...
/**
 * Check if the key passed in parameter is the key code. e.g if the parameter is '01' it will return true - but if it is
 ...
 */
const isKeyCode = key => /^\d+$/.test(key);
 ...
 */
const validateCheckDigit = (position, value) => {
 ...
 * @returns {boolean} true if the check digit is valid - false otherwise
 */
const validateIdentifierCheckDigit = dl => {
  ...
  const keyCode = isKeyCode(key) ? key : identifiers.find(i => i.ruleName === `${key}-value`).code;

  if (!keyCodeToPosition[keyCode]) return false; // the identifier hasn't any check digit

  return validateCheckDigit(keyCodeToPosition[keyCode], dl.identifier[key]);
};
